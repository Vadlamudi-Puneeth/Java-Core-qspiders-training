...
public class EmployeeOpsStream {
	public static void main(String[] args) {
		
//		List<Employee> list = getEmployee();
//		long noOfEmp = list.stream().filter(emp -> emp.getAge() > 20).count();
//		System.out.println(noOfEmp);
		...
		List<Employee> highEmp = getEmployee().stream().filter(emp -> emp.getSalary() == max).collect(Collectors.toList());
		System.out.println(highEmp);
	}
