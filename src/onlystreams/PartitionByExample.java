...
//		List<String> li = new ArrayList<>(Arrays.asList("apple", "orange", "kiwi", "mango"));
		
//		Map<Boolean, List<String>> map = li.stream()
//		.collect(Collectors.partitioningBy(i -> i.length() >= 3));
		
		List<Employee> list = getEmployee();
		
		Map<Boolean, List<Employee>> map = list.stream()
				.collect(Collectors.partitioningBy(i -> i.getGender().toLowerCase().equals("male")));
		
		for(Map.Entry<Boolean, List<Employee>> m: map.entrySet()) {
